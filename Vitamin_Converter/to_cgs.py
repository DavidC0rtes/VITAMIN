...
from Vitamin_Converter import initialisation
...
#check if every element of the lista is present inside listb
...
#This program take in entry an xml file describing an hypergraph and a file with the conditions that can be temporarly deactivated and output a cgs structure : states and their transitions  
def to_cgs(matrix, attacks, condition, cond_deactiv):
    ...
    while left_states != []:
        ...
        del left_states[0]
        #we take in account all the attacks to each states
        for attaque in attacks:
            #checking if the attack can be launched from the state
