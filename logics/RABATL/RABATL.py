...
# def Cost(actions, state):
#     total = 0
#     for action in actions:
#         total += get_cost_for_action(action, 's'+str(state))
#     return total
...
def Cost(actions, state):
    total = 0
    for action in actions:
        total += sum(get_cost_for_action(action, 's'+str(state)))
    return total
...
def pre(coalition, state_set, bound):
    ...
    for key, value in dict_state_action.items():
        ...
        for action in value:
            ...
            for k, v in other_actions_in_row.items():
                other_act = get_coalition_action(v, agents)
    ...
    for i in pre_states:
        ...
        for j, column in enumerate(graph[int(i)]):
            if graph[i][j] != 0:
                opponent_moves_in_row.update(get_opponent_moves(build_list(graph[i][j]), agents))

                if j in state_set:
                    # takes the opponent's moves for actions towards the states in state_set
                    opponent_moves_in_state.update(get_opponent_moves(build_list(graph[i][j]), agents))
