...
from utilities.parser.OATL import *
...
def next(action, state):
    ...
    for index, elem in enumerate(graph[state]):
        if graph[state][index] != 0:
            for elem_action in build_list(elem):
...
def D(s, agents, state_set):
    agents = get_agents_from_coalition(agents)
    tmp_set = set()
    tmp_set.add(s)
    ...
    for index, elem in enumerate(graph[state_index]):
        if graph[state_index][index] != 0:
            for action in build_list(graph[state_index][index]):
                base_action = get_base_action(action, agents)
...
def Cost(action_set, state):
    ...
    for action in action_set:
        total += get_cost_for_action(action, state)
    return total
